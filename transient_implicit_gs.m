function [ T, gs_iter] = transient_implicit_gs(nt,error,tol,nx,ny,T,Told,T_previous_nt,term1,term2,term3)
...
%time loop
  ...
  for k = 1:nt
   ...
   figure(3);
   title_text = sprintf('Error of transient heat conduction implicit equation using gauss seidel method=%d',gs_iter);
   title(title_text)
   ...
   pause(0.001)
    ...
    title_text = sprintf('Transient heat conduction implicit equation using gauss seidel method=%d',gs_iter);
    title(title_text)
    xlabel('x');
    ylabel('y');
