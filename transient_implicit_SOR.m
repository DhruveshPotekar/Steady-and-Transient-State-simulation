function [ T, sor_iter ] = transient_implicit_SOR(nt,error,tol,nx,ny,T,Told,alpha,T_previous_nt,term1,term2,term3)
...
%time loop
  ...
  for k = 1:nt
   ...
   figure(1);
   title_text = sprintf('Error of transient heat conduction implicit equation using successive over relaxation method=%d',sor_iter);
   title(title_text)
   ...
   pause(0.001)
    ...
    title_text = sprintf('Transient heat conduction implicit equation using successive over relaxation method=%d',sor_iter);
    title(title_text)
    xlabel('x');
    ylabel('y');
